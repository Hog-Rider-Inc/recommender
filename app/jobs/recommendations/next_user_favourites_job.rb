...
module Recommendations
  class NextUserFavouritesJob < ApplicationJob
    ...
    def existing_user_favourites_for(client)
      favourite_ids_from_favourites = client.client_favourites.select(:menu_item_id)
      favourite_ids_from_likes = ClientItemInteraction.where(client_id: client.id,
                                                             interaction: 'like').select(:menu_item_id)

      MenuItem
        .where(id: favourite_ids_from_favourites)
        .or(MenuItem.where(id: favourite_ids_from_likes))
        .distinct
        .to_a
    end
